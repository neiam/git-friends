...
impl MqttClient {
    ...
    pub async fn publish_commit(&self, commit_info: &CommitInfo, username: Option<&str>) -> Result<()> {
        let topic = if let Some(username) = username {
            format!("{}/{}/{}/{}", 
                self.config.topic_prefix, 
                username,
                commit_info.repository_url.replace("https://", "").replace("http://", "").replace("/", "_"),
    ...
    pub async fn wait_for_connection(&self) -> Result<()> {
        ...
        timeout(Duration::from_secs(10), async {
            loop {
                if self.client.try_publish("$SYS/test", QoS::AtMostOnce, false, "test").is_ok() {
                    break;
                }
...
pub fn format_commit_for_irc(commit_info: &CommitInfo) -> String {
    format!(
        ...
        commit_info.repository_url.split('/').last().unwrap_or("unknown"),
