...
use git_friends::{git::CommitInfo, mqtt::MqttClient, Config, Result};
use log::{error, info};
...
async fn main() -> Result<()> {
    ...
    let count: u32 = matches
        ...
        .parse()
        .map_err(|_| {
            git_friends::GitFriendsError::InvalidConfig("Invalid count value".to_string())
        })?;
    ...
    let interval: u64 = matches
        ...
        .parse()
        .map_err(|_| {
            git_friends::GitFriendsError::InvalidConfig("Invalid interval value".to_string())
        })?;
    ...
    loop {
        ...
        let commit_info = generate_mock_commit(repo_url, author, branch, iteration);
        ...
        if continuous {
            info!(
                "Sleeping for {} seconds... (Press Ctrl+C to stop)",
                interval
            );
...
fn generate_mock_commit(repo_url: &str, author: &str, branch: &str, iteration: u32) -> CommitInfo {
    ...
    let files_sets = vec![
        ...
        vec![
            ...
            "frontend/src/components/Login.js".to_string(),
        ],
